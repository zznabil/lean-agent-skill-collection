...
Before comparing interventions, start with the decision goal, the question that separates choices and the smallest measurable outcome. (Goal-Question-Metric). State the falsifiable hypothesis, control and intervention; inspect existing evidence first. A disposable prototype is a separating test, not production code.
...
For material dataset, model or AI comparisons, use [DATA-QUALITY.md](DATA-QUALITY.md) before accepting the result. Do not launch when authority, consent, privacy or recovery is unresolved.
