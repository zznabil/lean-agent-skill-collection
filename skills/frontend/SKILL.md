...
1. Inspect the product goal, intended users, representative tasks, context of use, content, brand, current design system, platform conventions, code stack, and supplied references.
...
5. For platform-native work, inspect current official samples and prefer native controls, semantics, and interaction patterns before custom controls or ARIA. Add ARIA only when native semantics are insufficient and verify the resulting keyboard, focus, name, role, state, and value behavior.
