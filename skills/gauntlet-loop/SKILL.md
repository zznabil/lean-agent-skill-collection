...
Each benchmark records ID, provenance, observable requirement, verification method, environment, hard gate or soft score, threshold, evidence path, status, confidence, and public or holdout class. Record the project’s standing Definition of Done separately when one exists. For engineering artifacts, select only relevant quality attributes: correctness, performance, compatibility, usability or accessibility, reliability, security, maintainability, portability, and safety.
...
9. **Retest:** rerun the failed benchmark, adjacent checks, and relevant regression set with fresh evidence. For a multi-step procedure, stop dependent steps at the first material mismatch. Revert a change that makes the total result worse.
