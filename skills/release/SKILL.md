...
4. Confirm version consistency, compatibility and migration notes, dependencies, licenses, generated artifacts, and clean working state. For migrations, verify the sequence and recovery path; destructive contraction comes last.
...
10. Inspect archives, permissions, stray files, debug settings, secrets, reproducibility, and checksums.
11. Verify branch base and final diff, then state the authorized disposition: review, merge, retain, or discard.
12. Produce a release packet with decision, version, changes, upgrade steps, known issues, evidence, skipped checks, artifacts, checksums, rollout and monitoring, rollback, branch disposition, and approval state.
