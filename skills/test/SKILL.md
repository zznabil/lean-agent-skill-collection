...
When choosing tests, trace each requirement to an observable check; prefer cheap isolated tests, add boundary checks where needed and use properties for invariant-rich inputs. (ISO/IEC/IEEE 29119; practical test pyramid; property-based testing). Use the existing test stack; do not mock away the behaviour being proved.
...
Control randomness, time, network and shared state. Cover relevant invalid input, failure, cancellation, retry and recovery. For API, security, UI, user-information, AI/data, lifecycle or regulated checks, use only the matching section of [BOUNDARIES.md](BOUNDARIES.md).
