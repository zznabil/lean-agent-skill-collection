---
name: wait-what
description: "Re-explain or simplify a confusing answer when the user explicitly asks for a clearer version."
---
...
For instructions, put prerequisites before action and expected results, position and recovery near the relevant step. Use stable terms and small coherent sections. Do not force headings, a quiz, childish language or an artificial reading level.

A requested voice, quotation, code, log, schema or legal/scientific wording keeps its meaning and format. Summary and TL;DR headings may follow a valid user preference, but must serve different purposes rather than repeat the same paragraph.

When reporting measurable work, use factual counts only. If a bar is requested, use 20 cells, name the track and show processed/total; report the acceptance verdict separately. For example: `Audit [##########----------] 50% (5/10) processed; verdict NOT JUDGED`. Do not invent a denominator or equate processed with passed.
